...
public interface ProductRepository extends JpaRepository<Product, UUID>, JpaSpecificationExecutor<Product> {
    ...
    @Modifying
    @Query("""
            UPDATE Product p SET p.category = :newCategory
            WHERE p.category.id = :oldCategoryId
            """)
    void changeAllProductCategory(@Param("oldCategoryId") UUID oldCategoryId, @Param("newCategory") Category newCategory);
}
